...
double Element::integrals(IntegralsType type, int currentTau) {
    ...
    switch (type) {
        case PREVIOUS_TAU_N1:
            for (int i = 0; i < SCHEMA; i++) {
                double r = N1(points[i]) * NOP1->r + N2(points[i]) * NOP2->r;
                result += (N1(points[i]) * NOP1->getTempAtIteration(currentTau - 1) +
                           N2(points[i]) * NOP2->getTempAtIteration(currentTau - 1)) * N1(points[i]) * r * weights[i];
            }
            break;
        case PREVIOUS_TAU_N2:
            for (int i = 0; i < SCHEMA; i++) {
                double r = N1(points[i]) * NOP1->r + N2(points[i]) * NOP2->r;
                result += (N1(points[i]) * NOP1->getTempAtIteration(currentTau - 1) +
                           N2(points[i]) * NOP2->getTempAtIteration(currentTau - 1)) * N2(points[i]) * r * weights[i];
            }
            break;
        default:
            result = -1;
